...
async function onRequestPost(context) {
  ...
  try {
    ...
    const finalIsDisabled = isDisabled !== void 0 ? isDisabled ? 1 : 0 : existing.isDisabled || 0;
    const finalPrice = price !== void 0 ? price : existing.price || 20;
...
async function onRequestGet5(context) {
  ...
  try {
    ...
    try {
      await env.DB.prepare("ALTER TABLE registrations ADD COLUMN paymentMethod TEXT").run();
    } catch (e) {
    }
...
async function onRequestPost5(context) {
  ...
  try {
    ...
    const { eventId, eventName, price, name, phone, guess, paymentMethod } = data;
    ...
    try {
      await env.DB.prepare("ALTER TABLE registrations ADD COLUMN timestamp TEXT").run();
    } catch (e) {
    }
    try {
      await env.DB.prepare("ALTER TABLE registrations ADD COLUMN paymentMethod TEXT").run();
    ...
    await env.DB.prepare(
      "INSERT INTO registrations (id, eventId, eventName, price, name, phone, guess, isPaid, isManualWinner, timestamp, paymentMethod) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?)"
    ).bind(id, eventId, eventName, price, name, phone, guess || "", timestamp, paymentMethod || "Online").run();
...
// ../.wrangler/tmp/pages-0TLJ6j/functionsRoutes-0.34819076498920554.mjs
var routes = [
  {
